...
> **Nomes:** Paulo Rogério de Jesus Junior   **Turma:** Arquitetura de Aplicações WEB   **Data:** 20 / 08 / 2026
...
Resposta: Get está como verbo, deveria ser substantivo
Como é uma consulta deveria ser um GET ao invés de POST
O arquivo utilizado é muito grande, deveria ser utilizado paginação
...
1. Qual(is) problema(s) de design vocês identificam?
Resposta: Ao invés de usar o GET, deveria ser utilziado o DELETE
A resposta, não deveria ser se o aluno não existe, pois se o mesmo não existir não
No ENDPOINT não pode conter verbo, e a interrogação tem que ser substituida por "/"
...
1. Qual(is) problema(s) de design vocês identificam?
Não possuí nome e nem curso do aluno
O retorno do status está errado, deveria ser 400
A dica está errado, não precisa buscar a lista pois iremos criar e não alterar
...
1. Qual(is) problema(s) de design vocês identificam?
Falta a informação de API no método

2. Seu redesenho (método + rota + status codes):

```text
GET api/escolas/1/turmas/3/alunos/25/matriculas/88/disciplinas/12
Retorna os dados da disciplina 12 da matrícula 88.
Para montar a URL o app precisa conhecer 5 IDs diferentes.
Resposta: 200 OK + JSON da disciplina.
```
...
1. Qual(is) problema(s) de design vocês identificam?
A requisição está errada pois falta o ID da matrícula
a resposta deveria 400 Bad Request

2. Seu redesenho (método + rota + status codes):
```text
GET /api/alunos/7/matriculas/88
Se o aluno 7 não existe, responde:
400 Bad Request + "<html><b>Erro: aluno nao existe!</b></html>"
O app mobile quebra tentando fazer parse do JSON.
```
...
1. Qual(is) problema(s) de design vocês identificam?
Método errado, deveria ser patch
Rota errada, pois utiliza verbo "atualizar" e não existe / para separa as informações
Resposta errada deveria ser 400 BAD REQUEST

2. Seu redesenho (método + rota + status codes):

```text
PATCH /api/NotaParcial/aluno/7/disc/12/nota/8.5
Atualiza SÓ a nota parcial da disciplina, sem body.
Todos os dados vão na query string.
Resposta: 200 OK + "OK".
```
...
Resposta: Adicionar um versionamento de API e ficariam da seguinte forma 
GET    /api/v2/alunos
POST   /api/v2/alunos
GET    /api/v2/alunos/7
DELETE /api/v2/alunos/7
GET    /api/v2/alunos/7/matriculas
GET    /api/v2/matriculas/88/disciplinas/12
PATCH  /api/v2/alunos/7/disciplinas/12/nota-parcial
